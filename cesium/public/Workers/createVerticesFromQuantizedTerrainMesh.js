define([
  ...
  ;(g.getRegularGridAndSkirtIndicesAndEdgeIndices = function (e, t) {
    ...
    (g.getEstimatedLevelZeroGeometricErrorForAHeightmap = function (e, t, r) {
      return (2 * e.maximumRadius * Math.PI * g.heightmapTerrainQuality) / (t * r)
    }),
    (g.prototype.requestTileGeometry = a.DeveloperError.throwInstantiationError),
  ...
  function C(e, t, n, o, a, s, d, h, u, l, I, g, m, T) {
    ...
    for (let r = 0; r < S; ++r) {
      ...
      let C, W
      o.hasWebMercatorT &&
        (C = (c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(x.latitude) - I) * g),
  ...
  return d(function (a, d) {
    ...
    m &&
      ...
      (O = 1 / (c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(D) - Y)))
    ...
    for (let e = 0; e < u; ++e) {
      ...
      ;(U[e] = new t.Cartesian2(o, a)),
        (z[e] = s),
        (q[e] = d),
        m && (Q[e] = (c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(x.latitude) - Y) * O),
    ...
    let ae
    if (F < 0) {
      ae = new n.EllipsoidalOccluder(k).computeHorizonCullingPointPossiblyUnderEllipsoid(_, q, F)
    }
    let se = F
    ;(se = Math.min(se, A(a.westIndices, a.westSkirtHeight, z, U, p, k, V, X, Z))),
      (se = Math.min(se, A(a.southIndices, a.southSkirtHeight, z, U, p, k, V, X, Z))),
      (se = Math.min(se, A(a.eastIndices, a.eastSkirtHeight, z, U, p, k, V, X, Z))),
      (se = Math.min(se, A(a.northIndices, a.northSkirtHeight, z, U, p, k, V, X, Z)))
